import {
  ...
  MAPA_LARGURA,
  type LoteMapa,
} from "@/lib/mapa/industrial-ponzoni";
 ...
 */
function enquadrar(destacados: LoteMapa[], proporcao: number): Caixa {
  ...
  x0 = Math.min(Math.max(x0, 0), MAPA_LARGURA - w);
  y0 = Math.min(Math.max(y0, 0), MAPA_ALTURA - h);
...
export default function MapaDaProposta({
  ...
  const destacados = LOTES_MAPA.filter((l) => ids.has(l.id));
  if (destacados.length === 0) return null;

  const caixa = enquadrar(destacados, proporcao);
  // o rótulo tem de continuar legível depois do recorte, então acompanha o zoom
  const escala = caixa.w / MAPA_LARGURA;
  ...
  return (
    <div className="mapa-proposta">
      ...
      >
        ...
        {LOTES_MAPA.filter((l) => !ids.has(l.id)).map((l) => (
          <polygon
            key={l.id}
            points={l.points}
      ...
      >
        ...
        />
        {destacados.map((l) => (
          <polygon key={l.id} points={l.points} fill="#E0A221" />
        ))}
        <rect
          x={caixa.x}
          y={caixa.y}
