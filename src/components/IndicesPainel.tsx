    ...
    <div className="flex flex-col gap-6">
      ...
      <section className="cartao p-4 flex flex-col gap-3">
        ...
        <p className="text-xs text-cinza">
          ...
          acumuladas no mês trazem o parcial até hoje, que não é a variação do
          mês fechado. Sem marcar &ldquo;substituir&rdquo;, meses já lançados
          ficam como estão — uma correção sua não é desfeita.
          {!SERIE_SGS[codigo] && (
            <>
              {" "}
              <strong>
              ...
              <p key={r.indexador} className="text-sm">
                ...
                {r.erro ? (
                  ...
                  <span className="text-verde">
                    {r.novos > 0 && `${r.novos} ${r.novos === 1 ? "mês novo" : "meses novos"}`}
                    {r.novos > 0 && r.atualizados > 0 && ", "}
                    {r.atualizados > 0 &&
      ...
      <section className="cartao p-4 flex flex-wrap gap-3 items-end">
        ...
        <p className="text-xs text-cinza flex-1 min-w-[240px]">
          Lançar de novo o mesmo mês substitui o número — é assim que a prévia
          vira o índice fechado.
        </p>
      </section>
                  ...
                  {doAno.map(({ comp, linha }) => (
                    <td key={comp} className="num p-0">
                      <button
                        ...
                        className={`w-full px-2 py-1.5 text-right tabular text-xs hover:bg-papel-alt ${
                          linha
                            ? Number(linha.variacao) < 0
                              ? "text-vermelho font-semibold"
                              : "text-tinta"
                            : "text-linha-forte"
                        } ${comp === mes ? "bg-dourado-fraco" : ""}`}
                      >
                        {linha
